...
MongoHelper.prototype.connect = function connect () {
  mongoose.connect('mongodb://' + this.server + '/' + this.db);
  console.log('connected to mongodb: ' + this.server + ' @ ' + this.db);
};

MongoHelper.prototype.disconnect = function disconnect () {
  mongoose.disconnect();
  console.log('disconnected from mongodb: ' + this.server + ' @ ' + this.db);
};

MongoHelper.prototype.clearDB = function clearDB (callback) {
